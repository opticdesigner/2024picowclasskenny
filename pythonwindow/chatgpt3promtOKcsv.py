...
class MainApp(tk.Tk):
    ...
    def create_widgets(self):
        ...
        # 右側：兩個 Matplotlib 圖形區域（上：填色圖；下：標準化光譜對比折線圖）
    ...
    def add_spectrum_widget(self, spectrum):
        # 每個光譜在左側顯示一個子框架，包含檔名、倍率調整控制元件及預覽圖
    ...
    def on_multiplier_change(self, spectrum, var):
        try:
            ...
            messagebox.showerror("倍率更新錯誤", str(e))
    ...
    def update_all_plots(self):
        ...
        try:
            ...
            cmap = matplotlib.cm.get_cmap("jet")  # 或自訂 colormap
            colors = [cmap(norm(w)) for w in wavelengths]
            # 利用 fill_between 繪製帶顏色漸變之圖形（模擬填色效果）
    ...
    def update_ppf_info(self, total_intensity=None, wavelengths=None):
        ...
        try:
            ...
            if total_intensity is None or wavelengths is None:
                ...
                # 計算各範圍內的 ppf 值累加，公式： n * intensity * factor
