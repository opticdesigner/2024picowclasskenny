...
class MainApp(tk.Tk):
    ...
    def update_ppf_info(self, total_intensity=None, wavelengths=None):
        ...
        try:
            ...
            self.ppf_total_var.set(f"总 PPF (400-700nm): {total_ppf:.2f}")
            self.ppf_blue_var.set(f"蓝光 PPF (400-499nm): {blue_ppf:.2f}")
            self.ppf_green_var.set(f"绿光 PPF (500-599nm): {green_ppf:.2f}")
            self.ppf_red_var.set(f"红光 PPF (600-700nm): {red_ppf:.2f}")
        except Exception as e:
