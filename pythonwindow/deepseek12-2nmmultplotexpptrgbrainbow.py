...
def wavelength_to_rgb(wavelength, gamma=0.8):
    ...
    # 根據可見光譜顏色分佈調整
    red = np.clip(1.5 - 4*np.abs(x - 0.25), 0, 1)
    green = np.clip(1.5 - 4*np.abs(x - 0.5), 0, 1)
    blue = np.clip(1.5 - 4*np.abs(x - 0.75), 0, 1)
    
    # 調整伽馬值
    rgb = (red**gamma, green**gamma, blue**gamma)
...
class MainWindow:
    ...
    def load_file(self, index):
        ...
        try:
            with open(filepath, 'r') as f:
                ...
                yy = np.linspace(0, max(y) if max(y) > 0 else 1, 256)
                ...
                im = ax.imshow(Z, cmap=spectrum_cmap, 
                              ...
                              alpha=0.6, interpolation='bicubic')
    ...
    def update_sum_plots(self):
        ...
        try:
            ...
            # 生成總和圖漸變層
            xx = np.linspace(400, 700, 256)
            yy = np.linspace(0, max(sum_mw) if max(sum_mw) > 0 else 1, 256)
            X, Y = np.meshgrid(xx, yy)
            Z = X
