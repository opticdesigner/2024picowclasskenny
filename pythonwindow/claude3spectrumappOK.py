# 請用python寫出一個windows運用程式,並可以顯示繁體中文
# 1. 有2個顯示區塊,第一個區塊是輸入要有幾個光譜輸入檔要輸入,範圍為1-10個,假設輸入數字為N
# 2. 輸入為N後,下面的區塊顯示出有N個空的座標圖,另外還有一個空的強度總和圖及總和強度正規化圖
# 3. 可以在這N個空的座標圖尚須取讀取可電腦內的 txt檔案,有一個讀取按鈕,按下後可以由電腦內選取讀取檔案
# 2. 檔案內第一個row內有"nm"及"mw"  2個標示名稱
# 3. "nm"的column內包含了400至700每個step為1,此為X軸
# 4. "mw"的column內包含了對應400至700的值,此為Y軸
# 5. 讀取後請畫出X,Y曲線圖,Y軸為"mw",X軸為"nm"
# 6. 再將所有光譜強度相加後,另外顯示一個總和的光譜圖
# 7. 在畫出光譜強度相加後在正規化,的相對光譜圖
